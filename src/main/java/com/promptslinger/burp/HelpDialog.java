...
public class HelpDialog extends JDialog {
    ...
    private static String body(String key) {
        ...
        case "quickstart": return
            ...
            + "<p>In Burp's Proxy, Repeater, or Target tree, right-click a request and choose <b>Send to PromptSlinger</b>. "
            + "The URL and method appear at the top of the panel in green.</p>"
            + "<p class='warn'>Load the <b>inference endpoint</b> — the request whose path ends in "
            + "<code>/chat/completions</code> (or your platform's equivalent). "
            + "Paths like <code>/api/v1/chats/{id}</code> are state-save calls that return chat history JSON, not an AI response. "
            + "Quick check: if the URL bar contains <code>/chats/</code> followed by a UUID, go back to Burp HTTP history and find the <code>/completions</code> request instead.</p>"
        ...
        case "enumerator": return
            ...
            + "<h2>JS resource extraction &amp; HTML intelligence</h2>"
            + "<p>When a scan finds HTML pages, an <b>Extract JS Resources (N pages)</b> button appears. "
            + "Click it to parse every <code>src</code>, <code>href</code>, <code>action</code>, and "
            + "<code>data-endpoint</code> / <code>data-url</code> / <code>data-api</code> attribute from those pages, "
            + "then probe each discovered path. Already-scanned paths are skipped automatically.</p>"
            + "<p>For each <b>JS file</b> found, the Summary column shows hardcoded API paths and credential-looking strings "
            + "detected in the script content.</p>"
            + "<p>For <b>HTML pages</b>, clicking the row in the results table shows a "
            + "<b class='good'>HIDDEN ELEMENT FINDINGS</b> section at the top of the Response Detail pane — "
            + "listing every <code>data-*</code> attribute found inside hidden elements "
            + "(<code>hidden</code>, <code>display:none</code>, <code>type=hidden</code>). "
            + "This surfaces things like <code>data-endpoint: /api/admin/reset</code> that would otherwise require "
            + "manually reading the page source.</p>"
            + "<h2>OpenAPI path import</h2>"
            + "<p>When a scan finds an OpenAPI or Swagger spec (<code>/openapi.json</code>, <code>/swagger.json</code>, etc.), "
            + "a <b>Scan OpenAPI Paths (N)</b> button appears in the bottom bar showing how many paths the spec defines. "
            + "Click it to immediately probe every path declared in the spec — no guessing required. "
            + "Results are added to the existing table alongside the initial scan.</p>"
            + "<h2>Actions</h2>"
            + "<ul>"
            + "<li><b>Scan OpenAPI Paths (N)</b> — appears after scan if a spec was found; probes all N declared paths</li>"
        ...
        case "sending": return
            ...
            + "<p>Right-click a request in Burp → <b>Send to PromptSlinger</b>. The request must have a JSON body "
            + "with a string field that carries the user message.</p>"
            + "<p class='warn'><b>Load the inference endpoint, not the state endpoint.</b> "
            + "Target the request whose path ends in <code>/chat/completions</code> (or your platform's inference path). "
            + "Endpoints like <code>/api/v1/chats/{id}</code> are UI bookkeeping calls — they return HTTP 200 with chat history JSON "
            + "but never call the model, so PromptSlinger will appear to succeed while receiving no AI response.</p>"
